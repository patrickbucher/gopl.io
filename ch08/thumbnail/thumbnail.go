...
// It returns the generated file name, e.g., "foo.thumg.jpg".
...
// ImageFile2 reads an image form the infile and writes
...
func ImageFile2(outfile, infile string) (err error) {
	...
	if err := ImageStream(out, in); err != nil {
		out.Close()
		return fmt.Errorf("saling %s to %s: %s", infile, outfile, err)
	}
	return out.Close()
}
